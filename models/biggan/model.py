...
class Generator(nn.Module):
    ...
    def forward(self, cond_vector, truncation=0.4, return_features=False, feature_layer=None):
...
class BigGAN(nn.Module):
    ...
    def forward(self, z, class_label, truncation=0.4, return_features=False, feature_layer=None):
        assert 0 < truncation <= 1

        embed = self.embeddings(class_label)
        cond_vector = torch.cat((z, embed), dim=1)

        z = self.generator(cond_vector, truncation=truncation, 
