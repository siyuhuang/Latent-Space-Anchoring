...
class Generator(nn.Module):
    ...
    def mean_std_latent(self, n_latent):
        latent_in = torch.randn(
            n_latent, self.style_dim, device=self.input.input.device
        )
        latent = self.style(latent_in)
        
        latent_avg = latent.mean(0, keepdim=True)
        latent_std = latent.std(0, keepdim=True, unbiased=True)
        return latent_avg, latent_std
    ...
    def forward(
        ...
        returns = [image]
        if return_latents:
            returns.append(latent)
        if return_features:
            returns.append(outs)
        if return_skips:
            returns.append(skips)
        if len(returns)<=1:
            returns.append(None)
...
class LinearDiscriminator(nn.Module):
    def __init__(self, input_dim=512, latent_avg=None):
        super().__init__()
        self.stddev_group = 4
        self.stddev_feat = 1
        if latent_avg is not None:
            if len(list(latent_avg.size()))>1:
                latent_avg = latent_avg[0:1]
        self.latent_avg = latent_avg
        
        self.linear = nn.Sequential(
            EqualLinear(input_dim+1, 128, activation='fused_lrelu'),
            EqualLinear(128, 1),
        )

    def forward(self, input):
        batch, channel = input.shape
